...
module "alb_dns_record" {
  source = "git::https://github.com/nexient-llc/tf-aws-module-dns_record.git?ref=0.1.0"
...
module "virtual_gateway" {
  source = "git::https://github.com/nexient-llc/tf-aws-module-appmesh_virtual_gateway.git?ref=0.1.0"
...
module "ecs_app_heart_beat" {
  ...
  naming_prefix      = "${var.naming_prefix}-hb"
  environment        = var.environment
  region             = var.region
